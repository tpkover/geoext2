...
Ext.define('GeoExt.data.LayerStore', {
	requires: ['GeoExt.data.LayerModel', 'Ext.Array',
		'Ext.data.Model'
	],
	...
	/**
	 * Return the index of the given layer in the map's layers array.
	 *
	 * @param layer
	 * @returns {number}
	 */
	...
	onLoad: function (store, records, successful) {
		if (successful) {
			...
			if (len > 0) {
				this._adding = true;
				for (var j = 0; j < len; j++) {
					this.map.addLayer(records[j].getLayer());
				}
				delete this._adding;
			}
		}
	 ...
	 */
	getRecordByLayer: function (layer) {
